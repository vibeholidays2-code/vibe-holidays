...
async function verifyPackage() {
  ...
  try {
    ...
    const charDhamPackages = await packages.find({ destination: 'Char Dham' }).toArray();
    
    console.log(`Found ${charDhamPackages.length} Char Dham package(s):\n`);
    
    charDhamPackages.forEach((pkg, index) => {
      ...
      console.log(`  Group Departures: ${pkg.groupDepartures?.length || 0} dates`);
