...
async function verifyShimlaPackage() {
  ...
  try {
    ...
    // Find all Shimla packages
    const shimlaPackages = await packages.find({ destination: 'Shimla' }).toArray();
    
    console.log(`Found ${shimlaPackages.length} Shimla package(s):\n`);
    
    shimlaPackages.forEach((pkg, index) => {
