...
async function verifyPackage() {
  ...
  try {
    ...
    const thailandPackages = await packages.find({ destination: 'Thailand' }).toArray();
    
    console.log(`Found ${thailandPackages.length} Thailand package(s):\n`);
    
    thailandPackages.forEach((pkg, index) => {
