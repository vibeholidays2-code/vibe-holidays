...
async function verifyPackage() {
  try {
    ...
    const pkg = await Package.findById('69c68316f04b113c43ec7ce0');
    ...
    console.log(`Price: ₹${pkg.price} per person (All Inclusive)`);
    console.log(`Active: ${pkg.active}`);
    console.log(`Featured: ${pkg.featured}`);
    console.log(`Category: ${pkg.category}`);
    console.log(`Tour Type: ${pkg.tourType}`);
    console.log(`Ideal For: ${pkg.idealFor}`);
    ...
    pkg.hotelDetails.forEach((hotel, i) => {
      ...
      console.log(`     Room: ${hotel.roomType}`);
    });
